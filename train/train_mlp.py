...
def parse_mlp_training_args():
    ...
    parser.add_argument('--train_data', type=str, required=True, help='Path to training data file (e.g., toy_data/toy_train_data.parquet)')
    ...
    parser.add_argument('--positive_class_weight', type=float, default=1.0, help='Weight for positive class in BCEWithLogitsLoss')
    parser.add_argument('--learning_rate', type=float, default=5e-4, help='Learning rate for optimizer')
    parser.add_argument('--l2_decay', type=float, default=3e-5, help='L2 weight decay for optimizer')
    parser.add_argument('--branch_hidden_dims', type=str, default='deeper', choices=['deeper', 'wider', 'shallower'], help='Branch hidden dimension preset: deeper, wider, shallower')
    parser.add_argument('--branch_embedding_dim', type=int, default=256, help='Output embedding size for each FP branch')
    parser.add_argument('--common_hidden_dims', type=int, nargs=2, default=[1024, 512], help='Hidden layer sizes for common MLP part (two integers)')
    parser.add_argument('--dropout_rate', type=float, default=0.4, help='Dropout rate applied in all MLP layers')
    parser.add_argument('--early_stopping_patience', type=int, default=10, help='Patience for early stopping')
    parser.add_argument('--n_splits', type=int, default=5, help='Number of CV splits')
    parser.add_argument('--use_scaffold_split', action='store_true', help='Use scaffold split (not implemented)')
    parser.add_argument('--grouping_strategy', type=str, default='auto', choices=['auto', 'bb', 'kmeans'], help='Grouping strategy for cluster-aware CV')
